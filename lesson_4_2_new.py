...
def get_simple_sieve(num):
    ...
    start = 2
    arr = list()
    steps_memo = {2: 0, 3: 1, 5: 3, 7: 5, 11: 9}

    while True:
        new_arr = [i for i in range(start, stop)]
        arr += new_arr
        stop = len(arr) * 2
        start = arr[-1] + 1

        for key, value in steps_memo.items():
            for i in range(value + key, len(arr), key):
                arr[i] = 0
                steps_memo[key] = i

        for i in arr:
            if i and i not in steps_memo and i < len(arr):
                steps_memo[i] = arr.index(i)

        simple_arr = [i for i in arr if i]
